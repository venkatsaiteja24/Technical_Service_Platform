  ...
  {technician.reviews && technician.reviews.length > 0 ? (
    <div>
      ...
      {/* <ul style={{ listStyle: 'none', padding: 0, fontSize: '0.85rem', color: '#555' }}>
        {technician.reviews.slice(0, 2).map((review, index) => (
          <li key={index} style={{ marginBottom: '5px' }}>
            <strong>{review.customerName}:</strong> "{review.comment}" - {review.rating}★ ({review.date})
          </li>
        ))}
        {technician.reviews.length > 2 && <li>...and more reviews</li>}
      </ul> */}
    </div>
  ) : (
